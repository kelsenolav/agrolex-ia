...
- **Contexto**: única feature que o usuário decidiu construir mesmo sem assinante pagante (recomendei esperar; ele escolheu seguir, ciente do trade-off). Rodado `/product-brainstorming` (no bloco "o que os especialistas propõem") e `/brainstorming` completo (decisões: substituição total do fluxo do Radar — Preference permanece só p/ planos/pacotes; cancelamento no app na v1 por postura CDC; critério de pronto = ciclo em sandbox). Spec: `docs/superpowers/specs/2026-07-01-mp-preapproval-radar-design.md`.
- **Implementação**:
  - `src/lib/payments/mercadopagoPreapproval.ts` *(novo, token lido lazy p/ testabilidade)* — `createPreapproval` (POST `/preapproval` com `status:"pending"`, sem cartão; cliente autoriza no redirect do `init_point`), `getPreapproval`, `getAuthorizedPayment`, `cancelPreapproval`. 11 testes.
  - `api/monitoring/checkout` — Radar troca Preference→Preapproval; grava `radar_subscriptions {status:'pending', mp_preapproval_id}` ANTES do redirect; `orders` sai do fluxo do Radar; fallback simulado (dev sem MP) mantido.
  - `api/webhook/mercadopago` — 2 ramos novos ANTES do fluxo atual: `subscription_preapproval` (authorized→active+expires; cancelled→cancelled+cancelled_at SEM mexer no período pago; paused→paused) e `subscription_authorized_payment` (aprovado→estende `expires_at` via `computeRenewalExpiry`; idempotência via marcador `authpay_<id>` em `mp_subscription_id`; cobrança aprovada reativa paused). Preapproval desconhecido→200 sem alterar nada (evita reentrega infinita). +7 testes.
  - `api/monitoring/subscription/cancel` *(novo)* — cancela só a assinatura do próprio usuário; **cancela no MP primeiro** e só marca no banco se o MP confirmou (nunca estado mentiroso onde o cliente segue sendo cobrado).
  - `radarSubscription.ts` — `getRadarStatus` deixa de filtrar `status='active'`: **'cancelled'/'paused' mantêm acesso até `expires_at`** (CDC — cancelar não corta o que foi pago; achado: a versão anterior cortaria acesso na hora do cancelamento). Expõe `auto_renew_active`/`subscription_status`.
  - `dashboard/radar/page.tsx` — banner de estado da assinatura (renovação automática ativa + botão cancelar com confirmação / cancelada com data-fim / pausada=cobrança falhou); banner "renove agora" suprimido quando auto-renew ativo.
  - `cron/renewals` — `shouldSkipManualRenewalReminder` *(pura, testada)*: preapproval ativo não recebe "renove manualmente" (MP cobra sozinho); `paused` recebe dunning; expirada pós-graça recebe win-back. Varredura inclui `paused`. `cron/monitoring` — gate de varredura inclui `cancelled`/`paused` até expirar (CDC).
- **🔴 2 BUGS LATENTES REAIS descobertos (schema de produção inspecionado de verdade, não suposto) e corrigidos ANTES de morderem** (`supabase/migrations/20260701_radar_subscriptions_user_unique.sql`, aplicada em prod via Management API + verificada): (1) **não existia UNIQUE em `radar_subscriptions.user_id`** — todos os `upsert(onConflict:'user_id')` do Eixo 2.1 (webhook, checkout) teriam falhado no PRIMEIRO pagamento real de Radar; (2) **coluna `updated_at` não existia** — webhook e cron já escreviam nela. Nunca explodiram porque a tabela sempre esteve vazia. Migration também adiciona `'paused'` ao CHECK de status.
- **🔴 DESCOBERTA OPERACIONAL (users/me na API real)**: o `MERCADO_PAGO_ACCESS_TOKEN` do Vercel, apesar do prefixo `APP_USR-`, pertence a um **TEST USER** do MP (`tags:["test_user"]`) — ambiente é sandbox de verdade, nenhum dinheiro real se move. Pra COBRAR DE VERDADE: gerar credenciais de produção da conta MP real do usuário e trocar a env var. Limites reais da API achados ao vivo: `reason` ≤40 chars (o do checkout tem ≤37, passa); pagador e cobrador precisam ser do mesmo "tipo" (test/real) — **o dono da conta MP não consegue assinar de si mesmo** (testar o checkout logado como advkelsenolavbruno falharia).
- **Prova ao vivo contra a API REAL do MP (não só mock)**: create→201 `pending` com `init_point` · get→200 · cancel→200 `cancelled` (assinatura de prova cancelada imediatamente, external_reference `e2e-proof-do-not-authorize`). A etapa "autorizar com cartão de teste no checkout hospedado do MP" não foi automatizada (login de test user no checkout do MP é fluxo manual) — coberta por 7 testes unitários do webhook; validar na primeira assinatura real.
- **Validação**: `tsc` 0, `lint` 0 erros (12 warnings pré-existentes), `jest` **755** (+20), `build` OK.
- **Deploy em produção**: **EFETUADO**.
...
- **Implementação (lembrete automático, e-mail apenas — SEM WhatsApp)**:
  - Confirmado por varredura no código: **não existe envio real de WhatsApp em lugar nenhum** — `cron/reminders/route.ts` só faz `console.log` simulando disparo. Único canal real é e-mail via Resend (mesmo padrão de `sendRadarRenewalReminder`). Não fabricamos um canal que não existe.
  - `src/lib/trial/trialReminder.ts` *(novo, puro)* — `classificarSegmentoTrial` (nunca_iniciou / iniciou_sem_concluir), `diasDesde`, `proximoMarcoLembrete` (marcos 3 e 10 dias, um por vez, recupera marco perdido em ordem). 13 testes.
  - `notificationService.ts` — nova `sendTrialReminderEmail` (mesmo padrão Resend/log_only), 2 variantes de copy (nunca iniciou vs. começou e não terminou).
  - `src/lib/trial/trialReminderJob.ts` *(novo)* — `runTrialReminderJob`: candidatos = leads com conta, não convertidos, cadastrados há ≥3 dias; exclui quem já tem análise concluída **OU** já teve `trial_completed` no histórico mesmo que a linha tenha sido apagada depois (gate duplo — evita dizer "você ainda não usou" pra quem já usou uma vez). Estado do marco enviado persiste em `metadata.trial_reminder` (sem migration, mesmo padrão de `interest_info`/`commercial_events`). 7 testes, incluindo o cenário exato achado ao vivo.
  - `cron/renewals/route.ts` — chama `runTrialReminderJob` no mesmo cron diário já existente (08h UTC) em vez de registrar um 3º job em `vercel.json` (evita depender do limite de crons do plano Vercel).
- **🔴 Decisão de segurança (não pedida, mas necessária)**: `RESEND_API_KEY` já está configurada em produção — ou seja, sem uma trava, o deploy já sairia mandando e-mail de verdade pra leads reais no próximo tick do cron. Adicionado gate explícito **`TRIAL_REMINDER_ENABLED`** (env var, default OFF): o código roda e é testável, mas **não envia nada** até o operador setar `TRIAL_REMINDER_ENABLED=true` no Vercel — mesmo padrão já usado para MP produção/CRON_SECRET. **Ação pendente do usuário**: setar essa env var quando quiser ativar de verdade.
- **🔴 Achado durante a verificação ao vivo (dry-run somente leitura, nunca disparou e-mail de verdade)**: rodado contra produção real — 1º achado confirma o diagnóstico (`angelopapa@gmail.com`/Angelo: seria selecionado, marco=3, segmento=`iniciou_sem_concluir` — exatamente o caso relatado). **2º achado, bug pego ANTES de subir**: `teste1@gmail.com` tem `trial_completed` no metadata (sucesso real, análise apagada depois) mas 0 linhas concluídas na tabela `analyses` hoje — minha 1ª versão da lógica o classificaria erroneamente como "nunca_iniciou" e mandaria "você ainda não usou sua análise gratuita" pra quem já usou. Corrigido com o gate duplo antes de qualquer deploy.
- **Validação**: `tsc` 0, `lint` 0 erros (mesmos 12 warnings pré-existentes), `jest` **725** (+21: 13+7 novos, +1 de ajuste), `build` OK.
- **Deploy em produção**: **EFETUADO** (código ativo, envio de e-mail desligado até `TRIAL_REMINDER_ENABLED=true`).
...
- **Pesquisa técnica (endpoints reais verificados ao vivo em 01/07/2026, nada suposto)**:
  1. **TerraBrasilis/DETER (INPE)** — serviço público **WFS/OGC sem autenticação**. Confirmado via `curl` real: `https://terrabrasilis.dpi.inpe.br/geoserver/{workspace}/wfs?...&outputFormat=application/json` retorna GeoJSON real e atual (alertas de jun/2026 em municípios de TO). Workspaces confirmados via `GetCapabilities`: `deter-amz` (Amazônia) e `deter-cerrado-nb` (Cerrado) — DETER (near-real-time) só cobre esses 2 biomas; schema (`DescribeFeatureType`) confirma campos `municipality`, `uf`, `view_date`, `classname`, `areamunkm`.
  2. **CAR/SICAR** — a API oficial (Conecta/Dataprev, "SICAR Imóvel") existe mas é **restrita a órgãos públicos** ("Setor Privado: Não" — confirmado via `gov.br/conecta`), inacessível para este produto. Porém o Radar **já tinha** `consultarCAR()` em `externalDataProviders.ts` batendo na consulta pública do `car.gov.br` — reaproveitado aqui em vez de duplicar.
  3. **IBAMA (embargos/autos de infração)** — **NÃO tem API pública filtrável**. Confirmado por 2 testes ao vivo: (a) `datastore_search` do CKAN em `dadosabertos.ibama.gov.br` retorna "Action name not known" — o dataset não está no datastore, é só arquivo estático; (b) o dataset de termos de embargo é um **.zip de 53MB** (`HEAD` confirmou `Content-Length: 53198850`) — inviável baixar/descompactar por requisição síncrona. O novo portal de consulta (`gov.br/ibama/.../autuacoes-e-embargos`) é web-manual, sem API. **Decisão**: não fabricar uma integração falsa — o embargo IBAMA continua autodeclarado, e o sistema agora AVISA isso explicitamente (nunca finge ter verificado).
- **Implementação**:
  - `externalDataProviders.ts` — nova `consultarDesmatamentoTerraBrasilis()`: consulta os 2 workspaces DETER em paralelo por `município+UF` (CQL_FILTER), retorna `verificado` (true mesmo com 0 alertas — distingue "checado e limpo" de "não checado") + lista de alertas com data/classe/área/fonte.
  - `environmentalEngine.ts` — `EnvironmentalInput` ganha `desmatamento_verificado_api`/`car_verificado_api`/`car_status` (proveniência real, nunca suposta); novo estado **`fonte_dados: 'parcial_api'`** (distinto de `autodeclarado` e do inatingível-por-ora `api_oficial`); trava por CAR cancelado/suspenso (não conforme) e pendente (verificação necessária); portão de "sem_dados" corrigido para não disparar quando uma API ao vivo checou e não achou nada (bug que existiria se só olhasse `alertas.length===0`); disclaimer sempre reafirma que o IBAMA segue autodeclarado mesmo em `parcial_api`.
  - `api/credito-rural/ambiental/route.ts` — antes de calcular, consulta TerraBrasilis (por `city`+`uf`) e reaproveita `consultarCAR` (por `car_code`) via `Promise.allSettled` (nunca bloqueia a resposta); enriquece o input do motor com os resultados reais; retorna `fontes_externas` no payload.
...
- **✅ RESOLVIDO (commits d21564f4 cache + 6a9895b1 OCR per-page)**: a 2.705 (6 págs) agora **CONCLUI numa passada com veredito correto 39/Crítico** (provado ao vivo). Duas causas-raiz reais (achadas por visibilidade local, não chute): **(1)** o "documento inteiro" do Gemini truncava e a cascata aprendia **OpenAI (lento, 123s)** p/ o per-page → corrigido: multi-página vai direto ao per-page com **sonda da pág.1** → usa **Gemini (46s)**, verificado LOCAL antes do deploy; **(2)** os e2e falhavam por **poluição de teste** — a rota faz OCR de TODOS os docs da propriedade (`route.ts:1205 .eq('property_id')`), e a propriedade de teste tinha 7 matrículas acumuladas → 42 págs → timeout. Com 1 doc, completa. Complementos: **cache de OCR resumível** (`findings.ocr_cache`, checkpoint antes da análise), **guardas de orçamento** (OCR e análise falham limpo, nunca orfaniza).
- **⚠️ Nota arquitetural (para escala)**: a rota faz OCR de **todos** os docs da propriedade a cada análise — uma propriedade com muitos docs fica lenta/pode estourar. Hoje o caso comum (1 matrícula) é rápido; se propriedades acumularem muitos docs, considerar escopar a análise aos docs relevantes.
- **⚠️ Higiene de teste**: os scripts e2e (`e2e-resume.mjs`, `e2e-2705.mjs`) criam docs na propriedade `efe70790…` (Fazenda teste) — **limpar a propriedade antes** de rodar, senão o acúmulo de docs distorce o teste (foi o que me confundiu).
- **Validação**: `tsc` 0, `lint` 0 erros, `build` OK, `jest` **693**. Deploys efetuados; e2e ao vivo **completed / 39-Crítico**.
